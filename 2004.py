...
def helpnote():
	...
	exit(" [*] Whatsapp :  https://wa.me/+8801624357535")

def notice():

 

	runtxt("\n\033[0;91m UNLIMITED PRIMIUM")

	runtxt("\033[0;93m SEND THIS KEY TO ADMIN >> %s%s"%(G,basesplit))

	runtxt("\033[0;92m ADMIN WHATSAPP >> 01624357535")

	subprocess.check_output(["am", "start", "https://wa.me/+8801624357535"])
...
basesplit = base4.replace('=', 'X').replace('A', '3').replace('B', '9').replace('C', '7').replace('D', '1').replace('E', '4').replace('M', '2').replace('L', '6').replace('F', '8').replace('N', 'E').replace('T', '8')
    ...
    │ [✓] WHATSAPP : +8801624357535   │

    │ [✓] POWER BY : \x1b[1;32mFERDOUS VAU\x1b[1;97m             │
    ...
    └───────────────────────────────────────┘  """)
		...
		print("%s [%s®%s] %sSTATUS    : %sUNLIMITED PRIMIUAM"%(G,R,G,Y,stat)) 

		print("\033[97;1m   ───────────────────────────────────────")

		print("\033[92;1m         ITS FERDOUS VAU      ")

		print("\033[97;1m   ───────────────────────────────────────")

		print("\n%s [%s1%s]%s CRACK RANDOM FB ID 2008-11 %s(FREE)"%(G,R,G,Y,W))

		print("%s [%s2%s]%s CRACK FB ID 2015-22 %s(100 Taka)"%(G,R,G,Y,G))

		print("%s [%s3%s]%s CRACK FROM NUMBER  %s(150 Taka) VIP"%(G,R,G,Y,G))

		print("%s [%s4%s]%s CRACK FB ID 2004-5%s(200 Taka) VIP-1"%(G,R,G,Y,G))

		print("%s [%s5%s]%s CRACK FROM EMAILS %s(KAJ KORE NA)"%(G,R,G,Y,G))

		print("%s [%s6%s]%s CRACK FB ID Custom %s(FREE) V1"%(G,R,G,Y,G))
